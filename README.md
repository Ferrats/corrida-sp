...
As zebras ficam dentro do asfalto; as áreas de escape e a grama ficam fora dele e reduzem a velocidade, sem anular a volta. Desenho, superfície e portais são derivados da mesma geometria em `src/game/track.ts`. Os recordes usam uma chave própria para o circuito e as regras de rua (`street-v1`). Os tempos do oval e das regras anteriores ficam preservados no navegador, mas não são carregados nem substituídos.

A filosofia é de corrida de rua: sair do asfalto já custa velocidade e tempo, sem punição adicional de volta inválida. Volte ao percurso e continue de onde parou. Todos os portais continuam obrigatórios, em ordem e no sentido correto, antes da chegada. O relógio continua correndo durante a saída e a recuperação. A superfície é determinada pelo centro do carro; esta versão não simula pneus individuais ou muros na pista.
...
- `R`: reposicionar no último portal aceito (mantém a volta, o tempo e o progresso; zera a velocidade).
